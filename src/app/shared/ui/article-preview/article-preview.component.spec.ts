...
describe(ArticlePreview.name, () => {
  ...
  describe('Given article', () => {
    ...
    it('Then render article author avatar with link', async () => {
      ...
      const avatarImg = getByAltText(
        /Avatar of article author/
      ) as HTMLImageElement;
    ...
    it('Then render toggle favorite button', async () => {
      ...
      expect(toggleFavoriteButton.nativeElement).toHaveTextContent(
        mockedArticle.favoritesCount.toString()
      );
      expect(toggleFavoriteButton.nativeElement).toHaveClassName(
        'btn-outline-primary'
      );
    });
    ...
    it('Then render article preview link', async () => {
      ...
      expect(previewLink.nativeElement).toHaveAttribute(
        'href',
        `/article/${mockedArticle.slug}`
      );
      expect(previewTitle.nativeElement).toHaveTextContent(mockedArticle.title);
      expect(previewParagraph.nativeElement).toHaveTextContent(

...
describe(AuthInterceptor.name, () => {
    ...
    async function setup(mockLocalStorage = false) {
        mockedLocalStorageService = jasmine.createSpyObj<LocalStorageService>(LocalStorageService.name, ['getItem']);

        if (mockLocalStorage) {
            mockedLocalStorageService.getItem.withArgs('ng-conduit-token').and.returnValue('token');
        }
    ...
    describe('Given there is a token', () => {
        it('Then a request is made with the auth header', async () => {
            ...
            const request = httpController.expectOne('/api/user');
            expect(request.request.headers.get('Authorization')).toEqual('Token token');
        });
    });
});

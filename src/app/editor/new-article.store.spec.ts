...
import { of } from 'rxjs';
import { ApiClient, Article, SingleArticleResponse } from '../shared/data-access/api';
import { AuthStore } from '../shared/data-access/auth.store';
import { getMockedArticle, getMockedProfile, getMockedUser } from '../testing.spec';
...
describe(NewArticleStore.name, () => {
    ...
    async function setup() {
        mockedApiClient = jasmine.createSpyObj<ApiClient>(ApiClient.name, ['createArticle']);
        ...
        describe('When createArticle', () => {
            function arrangeCreateArticle(article?: Article) {
                if (article) {
                    ...
                    mockedApiClient.createArticle.and.returnValue(of(null as unknown as SingleArticleResponse));
                }
            }
            describe('Given api.createArticle returns the article', () => {
                it('Then navigate to article page', async () => {
                    ...
                    expect(mockedRouter.navigate).toHaveBeenCalledWith(['/article', mockedArticle.slug]);
                });
            });
            ...
            describe('Given api.createArticle does not return the article', () => {
                it('Then navigate to user page', async () => {
                    ...
                    expect(mockedRouter.navigate).toHaveBeenCalledWith(['/profile', mockedUser.username]);
                });
            });
        });
    });

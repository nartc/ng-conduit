...
describe(EditArticleStore.name, () => {
    ...
    async function setup(getArticleState: 'success' | 'idle' = 'idle') {
        ...
        mockedApiClient = jasmine.createSpyObj<ApiClient>(ApiClient.name, ['getArticle', 'updateArticle']);
    ...
    describe('When update article', () => {
        ...
        it('Then navigate to the article page', async () => {
            ...
            expect(mockedRouter.navigate).toHaveBeenCalledWith(['/article', 'updated-slug']);
        });
    });
});

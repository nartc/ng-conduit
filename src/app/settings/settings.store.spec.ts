...
describe(SettingsStore.name, () => {
  ...
  async function setup() {
    mockedAuthStore = jasmine.createSpyObj<AuthStore>(
      AuthStore.name,
      ['authenticate', 'logout'],
      {
        auth$: of({
  ...
  describe('When updateSettings', () => {
    it('Then call apiClient.updateCurrentUser', async () => {
      ...
      mockedApiClient.updateCurrentUser.and.returnValue(
        of({ user: getMockedUser({ username: 'updated' }) })
      );
      ...
      expect(mockedAuthStore.authenticate).toHaveBeenCalledWith([
        '/profile',
        'updated',
      ]);
    });

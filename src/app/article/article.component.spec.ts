  ...
  describe('Given article', () => {
    ...
    describe('When render', () => {
      it('Then show banner', () => {
        ...
        const bannerContainerDiv = debugElement.query(
          By.css('.banner .container')
        );
      ...
      it('Then show article body', () => {
        ...
        expect(markdownParagraph.nativeElement).toHaveTextContent(
          vm.article?.body!
        );
      });
      ...
      it('Then show tags', () => {
        ...
        tagItems.forEach((tagItem, index) => {
          expect(tagItem.nativeElement).toHaveTextContent(
            vm.article!.tagList[index]
          );
        });
      });
      ...
      it('Then show comment form', () => {
        ...
        const commentForm = debugElement.query(
          By.directive(ArticleCommentForm)
        );

        expect(commentForm).toBeTruthy();
        expect(commentForm.componentInstance.currentUser).toEqual(
          vm.currentUser
        );
      });
    ...
    describe('When try to delete comment', () => {
      it('Then store.deleteComment is called with the comment', () => {
        ...
        comments.forEach((comment, index) => {
          ...
          expect(mockedStore.deleteComment).toHaveBeenCalledWith(
            vm.comments[index]
          );
        });
      });
    ...
    describe('When try to toggle follow author', () => {
      it('Then store.toggleFollowAuthor is called with article', () => {
        ...
        expect(mockedStore.toggleFollowAuthor).toHaveBeenCalledWith(
          vm.article!.author
        );
      });
    });

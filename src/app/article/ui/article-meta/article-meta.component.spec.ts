...
describe(ArticleMeta.name, () => {
    ...
    async function setup(isOwner = false, article: Parameters<typeof getMockedArticle>[0] = {}) {
    ...
    describe('Given is article owner', () => {
        it('Then render edit article link', async () => {
            ...
            const editArticleLink = debugElement.query(By.css('.btn.btn-outline-secondary'));

            expect(editArticleLink.nativeElement).toHaveAttribute('href', `/editor/${mockedArticle.slug}`);
        ...
        it('Then render delete article button', async () => {
            ...
            const deleteArticleButton = debugElement.query(By.css('.btn.btn-outline-danger'));

            expect(deleteArticleButton.nativeElement).toHaveTextContent('Delete Article');
        });
    ...
    describe('Given is not article owner', () => {
        it('Then render follow author button', async () => {
            ...
            expect(followAuthorButton.nativeElement).toHaveTextContent(new RegExp(mockedArticle.author.username));
        });
        ...
        describe('When click follow/unfollow', () => {
            it('Then followAuthor output emits with the article author', async () => {
                ...
                expect(mockedFollowAuthor.emit).toHaveBeenCalledWith(mockedArticle.author);
            });
        });
        ...
        it('Then render toggle favorite button', async () => {
            ...
            const toggleFavoriteButton = debugElement.query(By.css('#toggleFavorite'));
            const favoriteCountSpan = toggleFavoriteButton.query(By.css('span'));

            expect(favoriteCountSpan.nativeElement).toHaveTextContent(`(${mockedArticle.favoritesCount})`);
        });
        ...
        describe('Given author that has already been followed', () => {
            it('Then render Unfollow button', async () => {
                ...
                const followAuthorButton = debugElement.query(By.css('#followAuthor'));
                expect(followAuthorButton.nativeElement).toHaveTextContent(`Unfollow ${mockedArticle.author.username}`);
            });
        });
        describe('Given author that has not been followed', () => {
            it('Then render Follow button', async () => {
                ...
                const followAuthorButton = debugElement.query(By.css('#followAuthor'));
                expect(followAuthorButton.nativeElement).toHaveTextContent(`Follow ${mockedArticle.author.username}`);
            });
        });
        describe('Given article that has already been favorited', () => {
            it('Then render Unfavorite button', async () => {
                ...
                expect(toggleFavoriteButton.nativeElement).toHaveTextContent(/Unfavorite Post/);
            });
        });
        describe('Given article that has not been favorited', () => {
            it('Then render Favorite button', async () => {
                ...
                expect(toggleFavoriteButton.nativeElement).toHaveTextContent(/Favorite Post/);
            });
        });
    });
});

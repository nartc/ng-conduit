...
describe(ArticleStore.name, () => {
  ...
  async function setup(
    getArticleStatus: 'success' | 'failure' | 'idle' = 'idle'
  ) {
    ...
    if (getArticleStatus === 'success') {
      ...
      mockedApiClient.getArticleComments
        .withArgs(mockedSlug)
        .and.returnValue(of({ comments: mockedComments }));
    } else if (getArticleStatus === 'failure') {
      mockedApiClient.getArticle
        .withArgs(mockedSlug)
        .and.returnValue(throwError(() => 'error get article'));
      mockedApiClient.getArticleComments
        .withArgs(mockedSlug)
  ...
  describe('When toggleFavorite', () => {
    function arrangeToggleFavorite() {
      mockedApiClient.createArticleFavorite
        .withArgs(mockedSlug)
        .and.returnValue(
          of({ article: getMockedArticle({ article: { favorited: true } }) })
        );
      mockedApiClient.deleteArticleFavorite
        .withArgs(mockedSlug)
        .and.returnValue(
          of({ article: getMockedArticle({ article: { favorited: false } }) })
        );
    }
    ...
    describe('When article has already been favorited', () => {
      it('Then article is no longer favorited', async () => {
        ...
        store.toggleFavorite(
          getMockedArticle({ article: { favorited: true } })
        );
    ...
    describe('When article has NOT been favorited', () => {
      it('Then article is favorited', async () => {
        ...
        store.toggleFavorite(
          getMockedArticle({ article: { favorited: false } })
        );
  ...
  describe('When createComment', () => {
    ...
    function arrangeCreateComment() {
      mockedApiClient.createArticleComment.and.returnValue(
        of({ comment: mockedComment })
      );
    }
    ...
    it('Then new comment is added to comments', async () => {
      ...
      expect(mockedApiClient.createArticleComment).toHaveBeenCalledWith(
        mockedArticle.slug,
  ...
  describe('When deleteComment', () => {
    ...
    it('Then deleted comment is removed from comments', async () => {
      ...
      expect(mockedApiClient.deleteArticleComment).toHaveBeenCalledWith(
        mockedArticle.slug,
        mockedComments[0].id
      );
